...
CGT511C3::CGT511C3(int nPort, DWORD dwBaudrate)
{
	status = false;
	status = commport.Open(nPort, 9600);
...
int CGT511C3::open()
{
	...
	if( ReceiveData( uwDevID, (BYTE*)&gDevInfo, sizeof(gDevInfo), 100) < 0)
		return COMM_ERR;
...
int CGT511C3::captureFinger(BOOL bBest)
{
	return executeCmd( CMD_CAPTUREFINGER, bBest );
}

int CGT511C3::makeTemplate()
{
...
int CGT511C3::capture(BOOL best)
{
	return executeCmd( CMD_CAPTUREFINGER, best );
}

int CGT511C3::getImage()
{
	if( executeCmd( CMD_GETIMAGE, 0 ) < 0 )
		return COMM_ERR;
	
	if( ReceiveData( uwDevID, gbyImg256_tmp, sizeof gbyImg256_tmp, 10000 ) < 0 )
		return COMM_ERR;
...
int CGT511C3::getTemplate(int nPos)
{
...
DWORD CGT511C3::FirmwareVersion()
{
	return gDevInfo.FirmwareVersion;
}

DWORD CGT511C3::IsoAreaMaxSize()
{
	return gDevInfo.IsoAreaMaxSize / 1024;
}


int CGT511C3::executeCmd(WORD wCmd, int nCmdParam)
{
...
int CGT511C3::SendCmd( WORD wDevID, WORD wCmdOrAck, int nParam )
{
	CMD_PKT pkt;
	int nSentBytes;
...
int CGT511C3::ReceiveCmd( WORD wDevID, WORD* pwCmd, int* pnParam )
{
	CMD_PKT pkt;
	int nReceivedBytes;
...
int CGT511C3::SendData( WORD wDevID, BYTE* pBuf, int nSize )
{
	WORD wChkSum = 0;
	BYTE Buf[4], *pCommBuf;
	int nSentBytes;
...
int CGT511C3::ReceiveData(WORD wDevID, BYTE* pBuf, int nSize, DWORD timeout )
{
...
WORD CGT511C3::CalcChkSumOfCmd( CMD_PKT* pPkt )
{
	WORD wChkSum = 0;
	BYTE* pBuf = (BYTE*)pPkt;
	int i;
...
WORD CGT511C3::CalcChkSumOfDataPkt( BYTE* pDataPkt, int nSize )
{
	int i;
	WORD wChkSum = 0;
