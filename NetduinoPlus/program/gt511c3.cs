...
{  
    ...
    class CGT511C3
    {
        ...
        public devinfo gDevInfo;

        SerialPort serial;
        WORD uwDevID;
        DWORD CommTimeOut;

        public CGT511C3(int nPort, int dwBaudrate)
        {
            ...
            gDevInfo.DeviceSerialNumber = new BYTE[16];
            serial.Open();
        ...
        int open()
        {
	        ...
	        if (ReceiveData( uwDevID, ref Data, 24, 100) < 0 )
		        return COMM_ERR;

            gDevInfo.FirmwareVersion = (uint)Data[3] << 24 | (uint)Data[2] << 16 | (uint)Data[1] << 8 | Data[0];
            gDevInfo.IsoAreaMaxSize = (uint)Data[7] << 24 | (uint)Data[6] << 16 | (uint)Data[5] << 8 | Data[4];
    ...
    {
	    public DWORD FirmwareVersion;
	    public DWORD IsoAreaMaxSize;
